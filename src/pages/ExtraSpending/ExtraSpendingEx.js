...
const IncomeLog = () => {
  ...
  const getData = async () => {
    const res = await axios.get("http://localhost:5000/chitieuphatsinh");
    setData(res.data);
  };
        ...
        <section className="main">
          <section className="attendance">
            <div className="attendance-list">
              <h1>Quản lý danh sách chi tiêu phát sinh</h1>
              ...
              <Modal className="incomeModel" title="Thêm mới" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
                ...
                <label>Loại chi tiêu phát sinh</label>
                <input type="text" placeholder="" />
                <label>Số tiền</label>
                <input type="text" placeholder="" />
                  ...
                  {data?.map((value) => {
                    ...
                    return (
                      <tr>
                        <td>{value.id}</td>
                        <td>{value.ten}</td>
                        <td>{value.tenchitieuphatsinh}</td>
                        <td>{value.sotien}</td>
